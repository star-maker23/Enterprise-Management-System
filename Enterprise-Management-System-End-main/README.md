...
<p align="center">
 ...
 <a target="_blank" href="https://bladex.vip">
   <img src="https://img.shields.io/badge/Copyright%20-@EMS-%23ff3f59.svg" alt="Downloads">
 </a>
 </p>  
<p align="center">
...
<h1 align="center" style="margin: 30px 0 30px; font-weight: bold;">EMS v2.0.0 </h1>
...
EMS是一套开源的全栈精简快速开发平台，毫无保留给个人及企业免费使用。本项目的目标是做一款精简可靠，代码风格优良，项目规范的小型开发脚手架。
...
```
1. 生成所需的数据库表
找到后端项目根目录下的sql目录中的ems_xxxxx.sql脚本文件(取最新的sql文件)。 导入到你新建的数据库中。
...
4. 找到ems-admin模块中的EmsAdminApplication启动类，直接启动即可
...
#### 无Mysql/Redis 后端启动
```
1. 找到Enterprise Management System-admin模块下的resource文件中的application.yml文件
...
ems.embedded.mysql: false
ems.embedded.redis: false
改为
ems.embedded.mysql: true
ems.embedded.redis: true

请注意:高版本的MacOS系统，无法启动内置的Redis


3. 找到ems-admin模块中的EmsAdminApplication启动类，直接启动即可
```
...
``` 
ems
├── ems-admin -- 管理后台接口模块（供后台调用）
│
├── ems-api -- 开放接口模块（供客户端调用）
│
├── ems-common -- 精简基础工具模块
│
├── ems-infrastructure -- 基础设施模块（主要是配置和集成，不包含业务逻辑）
│
├── ems-domain -- 业务模块
├    ├── user -- 用户模块（举例）
├         ├── command -- 命令参数接收模型（命令）
...
- 项目基础环境搭建，请参考docker目录下的指南搭建。保姆级启动说明：
  - [AgileBoot - 手把手一步一步带你Run起全栈项目(SpringBoot+Vue3)](https://juejin.cn/post/7153812187834744845)
- 注意：管理后台的后端启动类是Ems**Admin**Application
